  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 py-4 md:py-6 transition-all duration-300 ${scrolled ? 'bg-white border-b-2 md:border-b-4 border-black' : 'bg-transparent'}`}>
      <div className="max-w-[1400px] mx-auto px-4 md:px-8 flex justify-between items-center">
        <div className="flex items-center gap-2 group cursor-pointer" onClick={() => vibrate(10)}>
      ...
      <div className={`fixed inset-0 bg-white z-[60] flex flex-col items-center justify-center gap-6 transition-all duration-500 md:hidden ${isOpen ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'}`}>
        <button onClick={() => { setIsOpen(false); vibrate(20); }} className="absolute top-6 right-6"><X size={32} /></button>
        {['ABOUT', 'SERVICES', 'CONTACT'].map((link) => (
          <a key={link} href={`#${link.toLowerCase()}`} onClick={handleClick} className="text-3xl font-display uppercase tracking-tighter hover:text-[#fb923c] transition-colors">{link}</a>
        ))}
        <a 
          href="#contact" 
          onClick={handleClick}
    ...
    >
      ...
      <div className="mt-auto">
        ...
        <div className="h-1.5 md:h-2 w-full bg-black/10 relative overflow-hidden rounded-full">
          <div className="h-full w-1/3 bg-black rounded-full transition-all group-hover:w-full duration-700"></div>
        </div>
      </div>
    </div>
  );
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          // Subtle haptic when section enters
          if (!entry.target.classList.contains('revealed-once')) {
            vibrate(8);
            entry.target.classList.add('revealed-once');
          }
        }
      });
    }, { threshold: 0.1 });
    ...
    const handleParallax = () => {
      if (heroRef.current) {
        ...
        if (h1) h1.style.transform = `translateY(${val}px)`;
      }
    };
  ...
  return (
    <div className="min-h-screen">
      {/* Audio Element - Low volume, lo-fi beat */}
      <audio 
        ref={audioRef} 
        loop 
      ...
      {!started && (
        <div id="enter-overlay">
          <div className="dot-loader mb-12"></div>
          <h2 className="text-4xl font-display uppercase tracking-tighter mb-8 text-center px-6">
            Sandeep Barupal <br/> <span className="text-[#fb923c]">Portfolio</span>
          </h2>
          <button 
            onClick={handleStart}
            className="group flex flex-col items-center gap-4 bg-black text-white px-12 py-6 rounded-2xl font-display text-xl uppercase tracking-[0.2em] shadow-[10px_10px_0px_#fb923c] active:translate-x-2 active:translate-y-2 active:shadow-none transition-all"
          >
            <Play className="group-hover:scale-125 transition-transform" fill="currentColor" />
            ENTER EXPERIENCE
          </button>
          <p className="mt-12 text-zinc-400 text-xs font-bold tracking-widest animate-pulse">SOUND ON RECOMMENDED</p>
        </div>
      )}
      ...
      {started && (
        <>
          ...
          <button 
            onClick={toggleMusic}
            className="fixed bottom-6 right-6 z-50 w-12 h-12 md:w-16 md:h-16 bg-white border-2 md:border-4 border-black rounded-full flex items-center justify-center shadow-[4px_4px_0px_black] hover:shadow-none hover:translate-x-1 hover:translate-y-1 transition-all"
          >
            {isMuted ? <VolumeX size={24} /> : <Volume2 size={24} className="animate-pulse" />}
          </button>

          {/* Hero Section with Parallax */}
          <section id="services" className="pt-28 md:pt-40 pb-12 md:pb-20 px-4 md:px-8" ref={heroRef}>
            <div className="max-w-[1400px] mx-auto">
              <div className="mb-10 md:mb-20 reveal-scroll active">
                <h1 className="text-4xl sm:text-6xl md:text-9xl font-display tracking-tighter uppercase mb-4 md:mb-6 leading-[0.9] md:leading-[0.8] parallax-item">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3 md:gap-8">
                {SKILLS_LIST.map((skill, idx) => (
                  <div key={skill.id} className="reveal-scroll" style={{ transitionDelay: `${idx * 50}ms` }}>
            ...
            <div className="max-w-[1400px] mx-auto grid md:grid-cols-2 gap-12 md:gap-20 items-center">
              <div className="reveal-scroll">
                ...
                <div className="flex gap-4">
                  <a href="#" onClick={() => vibrate(15)} className="w-10 h-10 md:w-14 md:h-14 rounded-full border-2 border-zinc-800 flex items-center justify-center hover:bg-white hover:text-black transition-all">
                    <Github size={20} />
                  </a>
                  <a href="#" onClick={() => vibrate(15)} className="w-10 h-10 md:w-14 md:h-14 rounded-full border-2 border-zinc-800 flex items-center justify-center hover:bg-white hover:text-black transition-all">
                    <Linkedin size={20} />
            ...
            <div className="max-w-[1400px] mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-8 md:gap-12">
              <div className="reveal-scroll">
                ...
                <a 
                  ...
                  onClick={() => vibrate(30)}
                  className="text-2xl md:text-6xl font-display uppercase tracking-tighter hover:text-zinc-500 transition-colors break-all"
                >
                  hello@sandeep.in
                </a>
              </div>
              
              <div className="w-full md:w-auto text-left md:text-right reveal-scroll">
                ...
                <div className="flex gap-4 md:gap-6 text-[10px] font-bold uppercase tracking-widest">
                  <a href="#" onClick={() => vibrate(10)} className="hover:underline">DRIBBBLE</a>
                  <a href="#" onClick={() => vibrate(10)} className="hover:underline">TWITTER</a>
                  <a href="#" onClick={() => vibrate(10)} className="hover:underline">LINKEDIN</a>
                </div>
              </div>
            </div>
          </footer>
